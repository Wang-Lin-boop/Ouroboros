...
def prepare(dataset, smiles_column="smiles", standardize=True):
    dataset = dataset.dropna(subset=[smiles_column])
    print(f"NOTE: read the dataset size ({len(dataset)}).")
    out_dataset = dataset.copy()
    if standardize == True:
        out_dataset[smiles_column] = out_dataset[smiles_column].apply(lambda x: gen_standardize_smiles(x, kekule=False, random=False))
    else:
        out_dataset[smiles_column] = out_dataset[smiles_column].apply(lambda x: check_smiles_validity(x))
    out_dataset = out_dataset[out_dataset[smiles_column]!='smiles_unvaild']
