...
class Fingerprint():
    ...
    def similarity(self, smiles1, smiles2, fingerprint_type=None, similarity_metric=None):
        if similarity_metric == None:
            similarity_metric = self.similarity_metrics_list[0]
    ...
    def similarity_predict(
        ...
        pred_values = {f'{fingerprint_type}_{key}':[] for key in similarity_metrics}
        ref_fgp = self.fingerprint_methods[fingerprint_type](Chem.MolFromSmiles(ref_smiles))
        for similarity_metric in similarity_metrics:
            pred_values[f'{fingerprint_type}_{similarity_metric}'] = fingerprint_database['features'].apply(lambda x: self.similarity_metrics[similarity_metric](ref_fgp, x))
        if as_pandas == True:
